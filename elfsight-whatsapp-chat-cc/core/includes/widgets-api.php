...
if (!class_exists('ElfsightWhatsappChatWidgetsApi')) {
    class ElfsightWhatsappChatWidgetsApi {
        ...
        public function __construct($slug, $file, $text_domain) {
            ...
            self::$ERROR_DOESNT_EXIST = esc_html__('Widget with the specified id doesnt exist.', $this->textDomain);
            self::$ERROR_PARAM_ID_REQUIRED = esc_html__('Parameter "id" is required.', $this->textDomain);
            self::$ERROR_DATA_INVALID = esc_html__('Incoming data is invalid.', $this->textDomain);
            self::$ERROR_SQL = esc_html__('A MySQL error occurred while adding a new widget.', $this->textDomain);
        ...
        public function getList(&$result) {
            ...
            if ($id > 0) {
                $query = $wpdb->prepare(
                    "SELECT * FROM $table_name WHERE `active` = %d AND `id` = %d ORDER BY `id` DESC",
                    1,
                    $id
                );
            } else {
                $query = "SELECT * FROM $table_name WHERE `active` = 1 ORDER BY `id` DESC";
            }
