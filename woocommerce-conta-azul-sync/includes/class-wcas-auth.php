...
class WCAS_Auth {
	...
	public function maybe_handle_callback(): void {
		...
		WCAS_Logger::diagnostic( 'oauth', 'callback_received', 'started', 'Callback OAuth2 recebido no admin.' );
		if ( ! current_user_can( 'manage_woocommerce' ) ) {
			wp_die( esc_html__( 'Você não tem permissão para conectar a Conta Azul.', 'woocommerce-conta-azul-sync' ) );
		}
		if ( isset( $_GET['error'] ) ) {
			WCAS_Logger::diagnostic( 'oauth', 'callback_error', 'error', 'OAuth callback retornou erro da Conta Azul.', array( 'error' => sanitize_text_field( wp_unslash( $_GET['error'] ) ), 'description' => sanitize_text_field( wp_unslash( $_GET['error_description'] ?? '' ) ) ) );
		...
		if ( false === $state_user_id || (int) $state_user_id !== get_current_user_id() ) {
			WCAS_Logger::diagnostic( 'oauth', 'validate_state', 'error', 'OAuth callback rejeitado por state inválido ou usuário divergente.', array( 'state_present' => '' !== $state ) );
		...
		WCAS_Logger::diagnostic( 'oauth', 'validate_state', 'success', 'State OAuth2 validado com sucesso.' );

		$result = $this->exchange_code( sanitize_text_field( wp_unslash( $_GET['code'] ) ) );
		$message = is_wp_error( $result ) ? 'auth_failed' : 'auth_success';
		WCAS_Logger::diagnostic( 'oauth', 'exchange_code', is_wp_error( $result ) ? 'error' : 'success', is_wp_error( $result ) ? $result->get_error_message() : 'Code OAuth2 trocado por tokens com sucesso.' );
	...
	private function token_request( array $body ): array|WP_Error {
		...
		if ( is_wp_error( $response ) ) {
			WCAS_Logger::diagnostic( 'oauth', 'token_request', 'error', $response->get_error_message(), array( 'grant_type' => $body['grant_type'] ?? '' ) );
			return $response;
		}
		...
		if ( $status < 200 || $status >= 300 || empty( $data['access_token'] ) || empty( $data['refresh_token'] ) ) {
			WCAS_Logger::diagnostic( 'oauth', 'token_request', 'error', 'Falha ao obter token OAuth2.', array( 'grant_type' => $body['grant_type'] ?? '', 'response' => $data ), $status );
		...
		WCAS_Logger::diagnostic( 'oauth', 'token_request', 'success', 'Token OAuth2 Conta Azul atualizado.', array( 'grant_type' => $body['grant_type'] ?? '', 'expires_in' => (int) ( $data['expires_in'] ?? 0 ) ), $status );
		return $data;
	}
}
