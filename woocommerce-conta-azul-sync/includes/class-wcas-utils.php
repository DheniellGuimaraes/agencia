	...
	public static function mask_sensitive( mixed $value ): mixed {
		if ( is_array( $value ) ) {
			...
			foreach ( $value as $key => $item ) {
				$masked[ $key ] = preg_match( '/token|secret|authorization|password|cpf|cnpj|document|email|phone|telefone/i', (string) $key ) ? self::mask_scalar( $item ) : self::mask_sensitive( $item );
			}
			return $masked;
		}
		return $value;
