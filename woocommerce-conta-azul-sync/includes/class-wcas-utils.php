...
class WCAS_Utils {
	 ...
	 * The current API uses Portuguese resource names; the legacy English paths return 404.
	 */
	...
	public static function defaults(): array {
		return array(
			...
			'detailed_log'            => 'no',
			'customer_search_path'    => '/pessoas',
			'customer_create_path'    => '/pessoas',
			'product_search_path'     => '/produtos',
			'product_create_path'     => '/produtos',
			'sale_create_path'        => '/vendas',
			'sale_cancel_path'        => '/vendas/{id}/cancelar',
			'receivable_create_path'  => '/financeiro/eventos-financeiros/contas-a-receber',
		);
	}
	...
	/**
	 * Migrate legacy English resource paths to the current Conta Azul API v2 paths.
	 *
	 * @param array<string, mixed> $settings Raw settings.
	 * @return array<string, mixed>
	 */
	private static function maybe_migrate_api_paths( array $settings ): array {
		$legacy_paths = array(
			'customer_search_path'   => array( '/customers' => '/pessoas' ),
			'customer_create_path'   => array( '/customers' => '/pessoas' ),
			'product_search_path'    => array( '/products' => '/produtos' ),
			'product_create_path'    => array( '/products' => '/produtos' ),
			'sale_create_path'       => array( '/sales' => '/vendas' ),
			'sale_cancel_path'       => array( '/sales/{id}/cancel' => '/vendas/{id}/cancelar' ),
			'receivable_create_path' => array( '/financial/receivables' => '/financeiro/eventos-financeiros/contas-a-receber' ),
		);

		$changed = false;
		foreach ( $legacy_paths as $key => $paths ) {
			$current_path = isset( $settings[ $key ] ) ? self::sanitize_path( (string) $settings[ $key ] ) : '';
			if ( isset( $paths[ $current_path ] ) ) {
				$settings[ $key ] = $paths[ $current_path ];
				$changed = true;
			}
		}
		...
		if ( $changed ) {
			...
			if ( class_exists( 'WCAS_Logger' ) ) {
				WCAS_Logger::diagnostic(
					...
					'api_paths_migrated',
					'success',
					'Endpoints legados em inglês migrados para os paths atuais da API v2 da Conta Azul.',
					array( 'migrated_keys' => array_keys( $legacy_paths ) )
				);
			}
		}
	...
	public static function get_settings(): array {
		...
		$settings = self::maybe_migrate_api_paths( $settings );
