    ...
    $(function () {
        ...
        function runAutoScanBatch() {
            ...
            $.post(SESAdmin.ajaxurl, {
                ...
                setAutoScanStatus(
                    ...
                    ' | Tempo: ' + (data.elapsed || 0) + 's' + syncMessage,
                    percent
                );
        ...
        function runAutoEnrichBatch() {
            ...
            $.post(SESAdmin.ajaxurl, {
                ...
                var sync = data.sitemap_sync || null;
                var syncMessage = sync ? (' | Sitemap URLs: ' + (parseInt(sync.urls, 10) || 0) + ' | Encontradas: ' + (parseInt(sync.matched, 10) || 0) + ' | Elegiveis: ' + (parseInt(sync.eligible, 10) || 0) + ' | Protegidas: ' + (parseInt(sync.protected, 10) || 0) + ' | Nao encontradas: ' + (parseInt(sync.missing, 10) || 0)) : '';
                ...
                setAutoEnrichStatus(
                    ...
                    ' | Enriquecidas: ' + autoEnrichStats.enriched +
                    ' | Erros: ' + autoEnrichStats.errors + (syncMessage || '') + '.',
                    100
                );
            }).fail(function () {
